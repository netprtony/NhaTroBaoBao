...
import { deleteRoom, checkDeleteRoom } from "@/app/(dashboard)/properties/[propertyId]/actions"
...
export function DeleteRoomDialog({ roomId, propertyId, roomCode, trigger }: DeleteRoomDialogProps) {
  ...
  const handleOpenChange = async (newOpen: boolean) => {
    ...
    if (newOpen) {
      ...
      setError(null)
      setCheckResult(null)
      const res = await checkDeleteRoom(roomId)
      if (res.error) {
        setError(res.error)
      } else if (res.data) {
          ...
          <div className="py-4 space-y-4">
            {checkResult?.allowed === false ? (
              <div className="bg-red-50 text-red-700 p-4 rounded-md flex gap-3 items-start">
                ...
                <div>
                  <p className="font-semibold">Không thể xóa phòng {roomCode}</p>
              ...
              <>
                ...
                {(checkResult?.blocking_count?.history_leases ?? 0) > 0 && (
                  <div className="bg-amber-50 text-amber-700 p-3 rounded-md text-sm border border-amber-200">
                    Phòng này có <strong>{checkResult?.blocking_count?.history_leases}</strong> hợp đồng trong lịch sử. Việc xóa phòng sẽ ẩn các thông tin lịch sử liên quan nhưng vẫn được lưu trữ trên hệ thống.
                  </div>
                )}
